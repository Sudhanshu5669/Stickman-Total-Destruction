...
/** Radius the stick saturates at, in CSS pixels before scaling. */
const STICK_R = 62;
...
export class TouchControls {
  ...
  draw(ctx: Ctx, w: number, h: number, playing: boolean) {
    ...
    // Button strip, bottom right: swap down / swap up, sized for a thumb.
    const bs = 58 * k;
    const pad = 16 * k;
    this.swapRight = { x: w - pad - bs, y: h - pad - bs, w: bs, h: bs };
    this.swapLeft = { x: w - pad - bs * 2 - 10 * k, y: h - pad - bs, w: bs, h: bs };
    this.pauseBtn = { x: pad, y: pad, w: 46 * k, h: 46 * k };
    ...
    if (this.move) {
      ...
      const s = Math.min(1, mag / (STICK_R * k)) * r;
      ...
      ctx.font = `900 ${11 * k}px "Trebuchet MS", system-ui, sans-serif`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("FLY", o.x, o.y - r - 12 * k);
    }
  }
  ...
  private button(ctx: Ctx, b: { x: number; y: number; w: number; h: number }, glyph: string, k: number, flash: number) {
    ...
    ctx.beginPath();
    ctx.roundRect(b.x, b.y, b.w, b.h, 14 * k);
    ctx.fill();
    ctx.strokeStyle = `rgba(255,255,255,${0.14 + flash * 0.5})`;
    ctx.lineWidth = 1.5 * k;
    ctx.stroke();
    ctx.fillStyle = "rgba(244,241,232,0.72)";
    ctx.font = `900 ${26 * k}px "Trebuchet MS", system-ui, sans-serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
  ...
  private pauseGlyph(ctx: Ctx, k: number) {
    ...
    ctx.beginPath();
    ctx.roundRect(b.x, b.y, b.w, b.h, 12 * k);
    ctx.fill();
    ctx.fillStyle = "rgba(244,241,232,0.7)";
    const bw = 5 * k;
    const bh = 18 * k;
    ctx.fillRect(b.x + b.w / 2 - bw - 3 * k, b.y + b.h / 2 - bh / 2, bw, bh);
    ctx.fillRect(b.x + b.w / 2 + 3 * k, b.y + b.h / 2 - bh / 2, bw, bh);
  }
}
